  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.success(
          "Registration successful! Redirectiong to login page....."
        );
        router.push("/login");
      }
    } catch (error) {
    ...
    const handleEdit = async () => {
        ...
        const response = await fetch(`/api/users?id=${props.userInfo.userId}`, {
            method: "PUT",
            body: data,
        });
        if (!response.ok) {
    ...
    <div className="max-w-sm mx-auto p-4 bg-[#1e1e1e] rounded-lg shadow-md">
      <form className="space-y-3" onSubmit={handleSubmit}>
        ...
        <div className="flex justify-center">
          ...
          >
            {props.buttonclicked === "edit" ? "Update" : "Register"}
          </button>
        </div>
      </form>
    </div>

...
//   // Everything after 'upload/' is the publicId (may include folders)
//   return parts
//     .slice(uploadIndex + 1)
//     .join("/")
...
// // Create Cloudinary instance using env variable
// const cld = new Cloudinary({
//   cloud: {
//     cloudName: process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || "prod",
//   },
// });

// function getUpscaledCloudinaryUrl(originalUrl: string, width: number, height: number) {
//   const publicId = extractPublicId(originalUrl);
//   if (!publicId) return originalUrl;
//   const img = cld.image(publicId);
//   img.resize(Resize.fill().width(width).height(height));
//   img.effect(Effect.upscale());
//   img.format("auto").quality("auto");
//   return img.toURL();
// }

function getCloudinaryUrl(originalUrl: string, width: number, height: number, dpr: number = 1) {
  if (!originalUrl.includes('res.cloudinary.com')) return originalUrl;
  return originalUrl.replace(
    '/upload/',
    `/upload/w_${width},h_${height},c_scale,dpr_${dpr},q_auto,f_auto,e_sharpen/`
  );
}
...
export default function TurfPage() {
  ...
  const [selectedDate, setSelectedDate] = useState<string>(() => {
    ...
    const bdOffsetMs = 6 * 60 * 60 * 1000; // 6 hours in milliseconds
    const bdTime = new Date(now.getTime() + bdOffsetMs);

    return bdTime.toISOString().split("T")[0]; // Format: YYYY-MM-DD
  });
  ...
  const handleSubmitReview = async () => {
    if (!reviewText || rating === 0 || !session?.user?.userId || !turf?.turfId)
      return;
  ...
  useEffect(() => {
    if (session?.user?.userId && reviews.length > 0) {
      const found =
        reviews.find((r) => r.userId === session.user.userId) || null;
      setUserReview(found);
    }
  }, [session, reviews]);
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      data.set(
        "endTime",
        selectedSlots[selectedSlots.length - 1].split(" - ")[1].trim()
      );
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1">
            <div className="w-full aspect-video relative rounded overflow-hidden">
              <Image
                src={getCloudinaryUrl(mainImage, 1000, 600, 2)}
                alt="Turf"
                width={1000}
                height={600}
            ...
            <div className="flex gap-2 mt-2">
              {turf.photos?.slice(0, 4).map((url, i) => (
                <Image
                  key={i}
                  src={getCloudinaryUrl(url, 80, 60, 2)}
                  alt={`thumbnail-${i}`}
                  width={80}
                  height={60}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-gray-400">
                          {userReview.date.toLocaleString()}
                        </span>
                      </div>
                    </div>
            ...
            <div className="mt-6">
              ...
              {reviews.length === 0 && (
                <p className="text-gray-400 mt-2">Be the first one to review!</p>
              )}
              {reviews.length !== 0 &&
              reviews.filter((r) => r.userId !== session?.user?.userId).length ===
                0 ? (
                <p className="text-gray-400 mt-2">
                  No reviews yet from other users.
                </p>
              ) : (
                reviews
          ...
          {session?.user && (
            <div className="w-full md:w-2/5 bg-[#1e1e1e] rounded p-4 flex flex-col items-center h-fit sticky top-5 self-start hidden lg:block">
              <p className="text-sm md:text-md text-gray-400 mb-2">
                Cost Per Hour:{" "}
                <span className="text-white font-bold">{turf.rate} bdt</span>
              </p>
              <hr />
              <label className="text-sm mt-4 text-white w-full">
                Select Date:
              </label>
